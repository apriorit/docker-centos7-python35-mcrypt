...
#define MCRYPTObject_Check(v)	(Py_TYPE(v) == &MCRYPT_Type)
...
get_iv_from_obj(MCRYPTObject *self, PyObject *ivobj, void **iv)
{
	if (ivobj == Py_None) {
		*iv = NULL;
	} else if (PyUnicode_Check(ivobj)) {
		int iv_size = PyUnicode_GET_SIZE(ivobj);
		*iv = PyUnicode_AsUTF8(ivobj);
...
MCRYPT__init__(MCRYPTObject *self, PyObject *args, PyObject *kwargs)
{
	...
	if (aobj == NULL) {
		...
		adir = NULL;
	} else if (PyUnicode_Check(aobj)) {
		adir = PyUnicode_AsUTF8(aobj);
	} else {
		PyErr_SetString(PyExc_TypeError,
	...
	if (mobj == NULL) {
		...
		mdir = NULL;
	} else if (PyUnicode_Check(mobj)) {
		mdir = PyUnicode_AsUTF8(mobj);
	} else {
		PyErr_SetString(PyExc_TypeError,
...
MCRYPT_encrypt(MCRYPTObject *self, PyObject *args, PyObject *kwargs)
{
	...
	if (catch_mcrypt_error(rc))
		...
		ret = PyBytes_FromStringAndSize(blockbuffer,
						 blockbuffer_size);
...
MCRYPT_encrypt_file(MCRYPTObject *self, PyObject *args, PyObject *kwargs)
{
	...
	while (1) {
		...
		data = PyUnicode_AsUTF8(result);
		data_size = PyUnicode_GET_SIZE(result);
...
MCRYPT_decrypt_file(MCRYPTObject *self, PyObject *args, PyObject *kwargs)
{
	...
	while (1) {
		...
		if (nextresult != NULL && PyUnicode_Check(nextresult)
		    && PyUnicode_GET_SIZE(nextresult) == 0)
			last = 1;
...
MCRYPT_get_key_sizes(MCRYPTObject *self, PyObject *args)
{
	...
	if (ret != NULL)
		for (i = 0; i != size; i++) {
			PyObject *o = PyLong_FromLong(key_sizes[i]);
			if (o == NULL) {
				PyObject_Del(o);
				ret = NULL;
				break;
...
{
	return PyLong_FromLong(self->block_mode);
}
...
get_dir_from_obj(PyObject *dirobj, char *default_dir, char **dir)
{
	if (dirobj == NULL) {
		...
		*dir = NULL;
	} else if (PyUnicode_Check(dirobj)) {
		*dir = PyUnicode_AsUTF8(dirobj);
	} else {
		PyErr_SetString(PyExc_TypeError,
...
_mcrypt_set_algorithm_dir(PyObject *self, PyObject *adirobj)
{
	if (adirobj == Py_None) {
		free(algorithm_dir);
		algorithm_dir = NULL;
	} else if (PyUnicode_Check(adirobj)) {
		free(algorithm_dir);
		algorithm_dir = strdup(PyUnicode_AsUTF8(adirobj));
...
_mcrypt_set_mode_dir(PyObject *self, PyObject *mdirobj)
{
	if (mdirobj == Py_None) {
		free(mode_dir);
		mode_dir = NULL;
	} else if (PyUnicode_Check(mdirobj)) {
		free(mode_dir);
		mode_dir = strdup(PyUnicode_AsUTF8(mdirobj));
		if (mode_dir == NULL) {
			PyErr_NoMemory();
			return NULL;
		}
...
_mcrypt_list_algorithms(PyObject *self, PyObject *args)
{
	...
	if (ret != NULL)
		for (i = 0; i != size; i++) {
			PyObject *o = PyUnicode_FromString(algorithms[i]);
			if (o == NULL) {
				PyObject_Del(ret);
				ret = NULL;
				break;
...
_mcrypt_list_modes(PyObject *self, PyObject *args)
{
	...
	if (ret != NULL)
		for (i = 0; i != size; i++) {
			PyObject *o = PyUnicode_FromString(modes[i]);
			if (o == NULL) {
				PyObject_Del(ret);
				ret = NULL;
				break;
